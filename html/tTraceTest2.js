...
let expName = 'tTraceTest2';  // from the Builder filename that created this script
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    if (trialMouse.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      trialMouse.x.push((Math.round(xys[0]*1000))/1000);
      trialMouse.y.push((Math.round(xys[1]*1000))/1000);
      trialMouse.time.push((Math.round(trialMouse.mouseClock.getTime()*1000))/1000);
    }
